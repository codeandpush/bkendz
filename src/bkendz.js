...
class Bkendz extends EventEmitter {
    ...
    connect(url, options = {}) {
        ...
        ws.http = function (url, opts = {}) {
            if (name === 'api') {
                url = self.apiLocation[location.protocol.slice(0, location.protocol.length - 1)] + (_.startsWith(url, '/') ? '' : '/') + url
            }
            return new Promise((resolve, reject) => {
                return $.ajax({
                    ...
                    // beforeSend: (xhr) => {
                    //     xhr.setRequestHeader('X-ACCESS-TOKEN', self.accessToken)
                    // },
                    error: reject, url
                })
            })
    ...
    http(url, opts = {}) {
        return new Promise((resolve, reject) => {
            return $.ajax({success: resolve, error: reject, url: url})
        })
    }
