...
const importer = new ModelImporter({config: require('<%= configRelPath %>'), configPath: '<%= configPath %>'})
let db = {}
...
class ModelImporter{
    ...
    constructor(opts){
        ...
        if (config.use_env_variable) {
            ...
            let storagePath = _.isString(opts.configPath) ? path.dirname(opts.configPath) : process.cwd()
    ...
    fromFile(filePath){
        ...
        function _defineCall(cls, ...rest) {
            return cls.getModel.apply(cls, rest)
        }
